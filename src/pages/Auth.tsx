          ...
          <Tabs defaultValue="login" className="w-full">
            ...
            <TabsContent value="login" className="space-y-4 mt-6">
              <div className="space-y-2">
                ...
                <Input id="login-email" type="email" placeholder="trader@example.com" value={email} onChange={(e) => setEmail(e.target.value)} className="bg-input border-border" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="login-password" className="text-foreground">{t("auth.password")}</Label>
                <Input id="login-password" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="bg-input border-border" onKeyDown={(e) => e.key === "Enter" && handleSubmit("login")} />
              </div>
              <Button onClick={() => handleSubmit("login")} className="w-full gradient-emerald hover:opacity-90" disabled={loading}>
            ...
            <TabsContent value="signup" className="space-y-4 mt-6">
              <div className="space-y-2">
                ...
                <Input id="signup-email" type="email" placeholder="trader@example.com" value={email} onChange={(e) => setEmail(e.target.value)} className="bg-input border-border" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="signup-password" className="text-foreground">{t("auth.password")}</Label>
                <Input id="signup-password" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="bg-input border-border" onKeyDown={(e) => e.key === "Enter" && handleSubmit("signup")} />
              </div>
              <Button onClick={() => handleSubmit("signup")} className="w-full gradient-emerald hover:opacity-90" disabled={loading}>
                {loading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}
